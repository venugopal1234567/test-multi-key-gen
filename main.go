...
func runKeyGen() {
	...
	for {
		...
		case msg := <-outCh:
			dest := msg.GetTo()
			if dest == nil {
				common.Logger.Fatal("did not expect a msg to have a nil destination during resharing")
			}
		...
		case save := <-endCh:
			...
			if save.Xi != nil {
				...
				common.Logger.Errorf("should not be an error getting a party's index from save data: %v", err)
				newKeys[index] = save
			} else {
				endedOldCommittee++
			}
			...
			if atomic.LoadInt32(&reSharingEnded) == int32(len(oldCommittee)+len(newCommittee)) {
				if len(oldCommittee) != endedOldCommittee {
					common.Logger.Warning("Invalid commitie provided")
				}
				common.Logger.Infof("Resharing done. Reshared %d participants", reSharingEnded)
				...
				for j, key := range newKeys {
					...
					BigXj := key.BigXj[j]
					if BigXj.Equals(gXj) {
						common.Logger.Infof("ensured BigX_j == g^x_j")
					}
				}
	...
	for {
		...
		case msg := <-signOutCh:
			...
			if dest == nil {
				...
				if dest[0].Index == msg.GetFrom().Index {
					common.Logger.Fatal("party %d tried to send a message to itself (%d)", dest[0].Index, msg.GetFrom().Index)
				}
		...
		case signData := <-signEndCh:
			...
			if atomic.LoadInt32(&signEnded) == int32(len(signPIDs)) {
				common.Logger.Infof("Signing done. Received sign data from %d participants", signEnded)
